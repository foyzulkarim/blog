...
The next module stays with libuv and unpacks the DNS call teased earlier — `dns.lookup` versus `dns.resolve`, why almost every library defaults to the thread-pool path, and the fan-out trap in a backend-for-frontend that calls many upstream services. V8 — how the engine runs the JavaScript the waiter carries, and what that costs — comes after that.
